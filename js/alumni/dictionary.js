...
$("#zdxz").change(function () {
    // 先清空字典名称查询
    $("#nameQuery").empty();
    var zdNow = $("#zdxz").val();
    if (zdNow) {
        $("#nameQuery").prop("disabled", false);
    } else {
        $("#nameQuery").prop("disabled", true);
    }
});
...
$('#myModa15').on('hidden.bs.modal', function () {
    ...
    $("#addZd").removeData("previousValue");
    $("#zdForm").removeData("previousValue");
    formValidator();
    formValidator2();
});
...
$("#saveInfo2").click(function () {
    var defalutForm = $("#addZd").data('bootstrapValidator');
    defalutForm.validate();
    if (defalutForm.isValid()) {
        $.ajax({
            url: "../addZd",
